...
- (void)generateBezierPaths {
    ...
    if (self.activeAreaShape != nil) {
        [self.activeAreaShape release];
        self.activeAreaShape = nil;
    }
    
    if (self.cropToCircle) {
        self.contentsLayer.cornerRadius = self.paletteDiameter / 2.0;
        self.activeAreaShape = [[UIBezierPath bezierPathWithOvalInRect:activeAreaFrame] retain];
    } else {
        self.contentsLayer.cornerRadius = 0.0;
        self.activeAreaShape = [[UIBezierPath bezierPathWithRect:activeAreaFrame] retain];
    }
